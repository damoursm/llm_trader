...
python main.py --schedule     # Start the poll-loop runner (RTH every 30 min 9:30–16:00 ET + extended trading ticks 4:00–20:00; NYSE market days; emails only from RTH slots)
...
**Extended hours (Phase 1 — TRADING, on by default).** `extended_hours_mode` = `off | observe | trade` (default **`trade`**). The runner ticks through the FULL extended day per `extended_windows` (default `04:00-07:00@60,07:00-09:30,16:00-17:30,18:00-20:00@60` ET — liquid shoulders every 30 min, thin dead zones hourly via the `@MM` per-window cadence suffix). In **trade** mode extended slots are FULL trading ticks (`_tick_plan` in `runner.py`): ledger entries/exits/marks and broker paper orders happen off-hours too; in **observe** mode (Phase 0) they run `run_pipeline(observe_only=True)` — full analysis + DuckDB persistence but zero ledger/broker mutation. **The daily email fires only from RTH slots in every mode** (extended slots past 16:00 must not re-send the report; the scheduler passes `email_if_configured=False` so its per-slot decision is authoritative — manual `python main.py` runs keep the email-if-configured convenience).
...
- **Order types:** `broker_order_type` = `MKT` (default) or `LMT` — a **marketable limit** at the model price ± `broker_limit_cap_bps` in the adverse direction (rounded away from the model to a valid tick), capping the worst acceptable fill on wide-spread names; an order whose cap is exceeded rests unfilled and is repaired by the fill-refresh pass. **Off-RTH ticks override the configured type**: IBKR rejects MKT outside regular hours, so extended-session submissions are always marketable LMT with `outsideRth=True` (`OrderRequest.outside_rth`); an order submitted overnight rests until the 04:00 pre-market open. Class-share tickers are normalised both ways (`to_ib_symbol`/`from_ib_symbol` in `ibkr.py`: yfinance `BRK-B` ↔ IBKR `BRK B`).
