...
**Method attribution** (`tracker.py`): every new trade stores the raw method scores (`news`, `sent_velocity`, `tech`, `insider`, `put_call`, `max_pain`, `oi_skew`, `vwap`, `pattern`, `momentum`, `money_flow`, `pead`, `iv_rank`, `iv_expr`, `coint`, `cross_sectional`) at entry time plus `methods_agreeing` and `dominant_method`. `get_performance_for_email()` returns:
...
|---|---|---|
| `record_new_trades()` | After Claude produces recommendations | Opens a trade per actionable BUY/SELL; stamps `entry_datetime` + `entry_price` together; enforces 3.0× per-sector cap; stores 10 raw method scores + `methods_agreeing` + `dominant_method`. |
